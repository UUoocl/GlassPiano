...
interface Props {
  onComplete: (config: { midiId: string, keyboard: KeyboardConfig, calibration: Calibration }) => void;
}
...
export const CalibrationWizard: React.FC<Props> = ({ onComplete }) => {
  ...
  const handleCalibrationComplete = (cal: Calibration) => {
    onComplete({
      midiId: selectedMidiId,
  ...
  return (
    <div className={`calibration-wizard absolute inset-0 z-[100] flex items-center justify-center p-4 ${step !== 'corners' ? 'bg-black/40 backdrop-blur-sm' : ''}`}>
      {step !== 'corners' ? (
        <div className="bg-white p-8 border-2 border-[#141414] shadow-[12px_12px_0px_0px_#141414] w-full max-w-lg animate-in fade-in zoom-in duration-300">
          {step === 'midi' && (
